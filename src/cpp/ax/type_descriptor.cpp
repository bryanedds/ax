...
namespace ax
{
    ...
    std::shared_ptr<type_descriptor> get_type_descriptor(std::type_index type_index)
    {
        ...
        if (type_descriptor_iter == std::end(type_descriptor_map))
        {
            ...
            return std::make_shared<inspectable_descriptor>();
        }
    ...
    {
        val& type_descriptor = get_type_descriptor<inspectable>();
        read_value_vptr(*type_descriptor, source_symbol, static_cast<void*>(&target_inspectable));
    }

    void write_value(const inspectable& source_inspectable, symbol& target_symbol)
    {
        val& type = get_type(source_inspectable);
        val& type_index = get_type_index(*type);
        val& type_descriptor = get_type_descriptor(type_index);
        write_value_vptr(*type_descriptor, static_cast<const void*>(&source_inspectable), target_symbol);
    }

    /* inspectable_descriptor */

    void inspectable_descriptor::inspect_value_impl(const void*, void*) const
    {
        throw std::runtime_error("inspectable_descriptor::inspect_value_impl not implemented.");
    }

    void inspectable_descriptor::inject_value_impl(const void*, void*) const
    {
        throw std::runtime_error("inspectable_descriptor::inject_value_impl not implemented.");
    }

    static void read_value_internal(const std::shared_ptr<type_t>& type, const symbol::right_type& symbol_tree, inspectable& inspectable)
    {
        // read sub-type values
        if (val* base_type_index = get_base_type_index_opt(*type).get())
        {
            val& base_type = get_type(*base_type_index);
            read_value_internal(base_type, symbol_tree, inspectable);
        }
        ...
        for (val& field_kvp : field_vector)
        {
            if (symbol_tree_iter != symbol_tree_end)
            {
                ...
                val& field_ptr = static_cast<char*>(static_cast<void*>(&inspectable)) + get_value_offset(*field);
    ...
    void write_value_internal(const std::shared_ptr<type_t>& type, const inspectable& inspectable, symbol::right_type& symbol_tree)
    {
        // write sub-type values
        if (val* base_type_index = get_base_type_index_opt(*type).get())
        {
            val& base_type = get_type(*base_type_index);
            write_value_internal(base_type, inspectable, symbol_tree);
        }
        ...
        for (val& field_kvp : field_vector)
        {
            ...
            val& field_type_index = get_type_index(*field);
            val& field_type_descriptor = get_type_descriptor(field_type_index);
            val& field_ptr = static_cast<const char*>(static_cast<const void*>(&inspectable)) + get_value_offset(*field);
            write_value_vptr(*field_type_descriptor, field_ptr, field_symbol_mvb);
            symbol_tree.emplace_back(std::move(field_symbol_mvb));
        }
    }

    void inspectable_descriptor::write_value_impl(const void* source_ptr, symbol& target_symbol) const
    {
        ...
        val* inspectable_ptr = static_cast<const inspectable*>(source_ptr);
        val& inspectable = *inspectable_ptr;
        val& type = get_type(inspectable);

        // write values
        write_value_internal(type, inspectable, symbol_tree);
    }

    void inspectable_descriptor::read_value_impl(const symbol& source_symbol, void* target_ptr) const
    {
        var* inspectable_ptr = static_cast<inspectable*>(target_ptr);
        match2(source_symbol,
        [&](val& symbol_tree)
        {
            var& inspectable = *inspectable_ptr;
            val& type = get_type(inspectable);
            read_value_internal(type, symbol_tree, inspectable);
        },
